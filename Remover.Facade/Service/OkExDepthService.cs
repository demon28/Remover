...
{
    public class OkExDepthService : FacadeBase, IJob
    {
        public bool RunOkExDepthService()
        {
            Log.Info("-----------------开始执行RunOkExDepthService-------------------");
            try
            {
                Vmp_ConfigCollection coinfig = new Vmp_ConfigCollection();
                if (!coinfig.ListByPlatformCode("OKEX"))
                {
            ...
            Log.Info("-----------------结束执行RunOkExDepthService-------------------");
            return true;
        }

        public JobResult Run(DateTime runTime)
        {
            if (!RunOkExDepthService())
            {
