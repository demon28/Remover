...
* FileName : Tmp_Platform.generate.cs 
* Version : V 1.1.0
* Author：架构师 曾杰(Jie)
* E_Mail : 6e9e@163.com
...
* CreateTime : 2018-07-18 20:10:25  
* 
***************************************************/
using System;
...
{
    /// <summary>
    /// Data Access Layer Object Of Tmp_Platform
    /// </summary>
    public partial class Tmp_Platform : DataAccessBase
    {
        #region 默认构造

        public Tmp_Platform() : base() { }

        public Tmp_Platform(DataRow dataRow)
            : base(dataRow) { }
        ...
        public const string _TABLENAME="tmp_platform";
        ...
        {
            string sql = @"delete from tmp_platform where " + condition;
...
values(
  ...
  ?remark)";
        ...
        protected bool UpdateByCondition(string condition)
        {
            ...
            sql.AppendLine("update tmp_platform set");
...
where " + condition;
    ...
    /// <summary>
    /// Data Access Layer Object Collection Of Tmp_Platform
    /// </summary>
    public partial class Tmp_PlatformCollection : DataAccessCollectionBase
    {
        #region 默认构造
 
        public Tmp_PlatformCollection() { }

        public Tmp_PlatformCollection(DataTable table)
            : base(table) { }
        ...
        {
            return new Tmp_Platform(DataTable.Rows[index]);
        }
        
        protected override DataTable BuildTable()
        {
            return new  Tmp_Platform().CloneSchemaOfTable();
        }
        
        protected override string TableName
        {
            get { return Tmp_Platform._TABLENAME; }
        }
...
where " + condition;
        ...
        #region 公开成员
        public Tmp_Platform this[int index]
        {
            get
            {
                return new Tmp_Platform(DataTable.Rows[index]);
        ...
        public Tmp_Platform Find(Predicate<Tmp_Platform> match)
        {
            foreach (Tmp_Platform item in this)
            {
                if (match(item))
                    return item;
            }
        ...
        public Tmp_PlatformCollection FindAll(Predicate<Tmp_Platform> match)
        {
            Tmp_PlatformCollection list = new Tmp_PlatformCollection();
            foreach (Tmp_Platform item in this)
            {
                if (match(item))
                    list.Add(item);
        ...
        public bool Contains(Predicate<Tmp_Platform> match)
        {
            foreach (Tmp_Platform item in this)
            {
                if (match(item))
        ...
        public bool DeleteAt(Predicate<Tmp_Platform> match)
        {
            BeginTransaction();
            foreach (Tmp_Platform item in this)
            {
